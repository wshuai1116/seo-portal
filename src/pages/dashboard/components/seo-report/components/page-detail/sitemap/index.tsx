...
import { generateSitemap } from "@/query/analysis/useGenerateSitemap";
...
function Sitemap() {
  const METRICS_CATEGORY = "SITEMAP";
  ...
  const Editor = ({
    ...
    const handleGenerateSitemap = () => {
      generateSitemap(item.metricsId)
        .then(({ result }) => {
          saveCommentDataMutation.mutate({
            itemId: item.itemId,
            ...
            <SharedButtonGroup>
              {isCommentExist && !editSwitch ? (
                <>
                  ...
                  <SharedButton
                    onClick={() => {
                      let url = "/api/sitemap/download?itemId=" + item.itemId;
                      axios({
                        url: url,
                        method: "POST",
                        responseType: "blob",
  ...
  return (
    <SharedRoot>
      <SharedTitle>sitemap.xml</SharedTitle>
      <SharedMainContainer>
        <MetricsSuggestion
          metricsCategory={METRICS_CATEGORY}
        ...
        {item ? (
          <Editor
            item={item}
            commentValue={item.commentData?.sitemap}
            defaultValue={item.data?.sitemap}
          />
        ) : null}
      </SharedMainContainer>
    </SharedRoot>
